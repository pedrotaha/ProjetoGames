...
public class CarrinhoLocacaoData {
        ...
        if (rsa.next()) {
            if (rsa.getBoolean("exists")) {
                ...
                if (rs1.next()) {
                    ...
                    String sql2 = "select quantidadealugar from tbquantidade where idjogo = " + jogoOp.getJogosModel().getIdJogos() + ";";
                    PreparedStatement ps2 = c.getConexao().prepareStatement(sql2);
                    ResultSet rs2 = ps2.executeQuery();
                    while (rs2.next()) {
                        quantidadeEstoque = rs2.getInt("quantidadealugar");
    ...
    public boolean esvaziarCar(UsuarioModel obj) throws Exception {
        Conexao c = new Conexao();
        c.getConexao().setAutoCommit(false);
        String sql1 = "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'tbjogolocad_tmp" + obj.getId() + "');";
        PreparedStatement ps1 = c.getConexao().prepareStatement(sql1);
        ResultSet rs1 = ps1.executeQuery();
        if (rs1.next()) {
            if (rs1.getBoolean("exists")) {
                ...
                PreparedStatement psa = c.getConexao().prepareStatement(sqla);
                psa.executeUpdate();
                String sql5 = "drop table if exists tbjogolocad_tmp" + obj.getId() + ";";
